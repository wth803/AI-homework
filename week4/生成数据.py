...
if not deepseek_api_key:
    raise ValueError("There is no deepseek api key===========")
...
for 卦象输入 in 卦象输入列表:
    ...
    content = lines[0].split(':"')[1].replace('"', '').strip() 
    summary = '\n'.join(lines[1:]).split(':"')[1].replace('"', '').strip() 
...
# 步骤6：保存为 CSV 文件
output_file = './generate_data/zhouyi_generate_dataset_deepseek.csv'

with open(output_file, 'w', encoding='utf-8', newline='') as f:
    writer = csv.DictWriter(f, fieldnames=['content', 'summary'])
    writer.writeheader()  
    writer.writerows(训练数据列表)  
